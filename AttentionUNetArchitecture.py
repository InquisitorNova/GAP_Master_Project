...
class ResidualDense_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_rate,
        ...
        self.conv3 = nn.Conv2d(in_channels + 2*out_channels, kernel_size=3, padding=1, stride=1)
        self.conv4 = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding = 0, stride=1)
        self.conv5 = nn.Conv2d(in_channels + 3*out_channels, kernel_size=1, padding = 0, stride=1)
...
class Down_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, time_channels, n_groups,
                ...
                Pooling = True, bottleneck_factor = None, units = None, 
...
class Up_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, time_channels, n_groups,
                ...
                bottleneck_factor = None, units = None, bottleneck_units = None,
                psnr_enabled = True):
        ...
        if up_sample:
            if "concat" in merge_type:
                ...
                if "Efficient_Residual_Block" in self.conv_type:
                    self.Conv_Block = Efficient_Residual_Block(in_channels = 2*in_channels, bottleneck_factor=bottleneck_factor, time_channels= time_channels, 
                                                       out_channels= out_channels, dropout_rate=dropout_rate, n_groups=n_groups, psnr_enabled = psnr_enabled)
            
                if "SqueezeExtraction_Block" in self.conv_type:
                    self.Conv_Block = SqueezeExtraction_Block(in_channels = 2*in_channels, out_channels= out_channels, units = units, dropout_rate= dropout_rate, 
...
class AttentionUNet(pL.LightningModule):
    ...
    """
    def __init__(self, initial_channels, channels_list, blocks_per_channel, n_groups, n_heads, dim_k, reduction_factor, num_groups, dropout_rate, time_channels, 
                 bottleneck_factor, units, bottleneck_units, conv_type, attn_type_list, merge_type, upsample_type, maxpsnr, minpsnr, 
                 num_timesteps, middle_attn_type, psnr_enabled = True, image_channels = 1):
