...
class BinomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, channels, windowSize, minPSNR, maxPSNR, virtSize = None, augment = True, maxProb = 0.99):
    ...
    def __getitem__(self, idx):
        ...
        psnr_map = psnr.unsqueeze(-1).unsqueeze(-1).expand(img.shape).type(torch.float32)
        out = torch.cat((img, imgNoise), dim = 0)

        if not self.augment:
            return img, psnr_map, imgNoise
            ...
            if np.random.rand() <0.5:
                ...
                img_target = out[:self.channels,...]
                img_input = out[self.channels:,...]
                return img_input, psnr_map, img_target

            else:
                out =  self.scale(self.rotate(out))
                img_target = out[:self.channels,...]
                img_input = out[self.channels:,...]
                return img_input, psnr_map, img_target
