...
class FacesDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, names, minPSNR, maxPSNR, augment = True, 
                 maxProb = 0.99, virtSize = None, grayscale = True):
    ...
    def __getitem__(self, idx):
        ...
        level = torch.min(level, torch.FloatTensor([self.maxProb]))
        Binomial = torch.distributions.Binomial(total_count = image, probs = level)
        
        ImageNoise = Binomial.sample()
        image_target = (image - ImageNoise).type(torch.float32)
        image_target /= (image_target.type(torch.float32).mean().item() + 1e-8)
    ...
    def show_transformed_image(self, index):
        ...
        ImageNoise, image, psnr_map = self.__getitem__(index)
